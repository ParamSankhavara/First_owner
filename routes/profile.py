...
async def get_builder_profile(request : Request):
    ...
    if len(user_data) != 0:
        count_data = [i._asdict() for i in db1.execute(text(f"""SELECT SUM(CASE WHEN current_state = 1 THEN 1 ELSE 0 END) AS completed_projects,SUM(CASE WHEN current_state = 2 THEN 1 ELSE 0 END) AS running_projects,SUM(CASE WHEN current_state = 3 THEN 1 ELSE 0 END) AS upcoming_projects FROM projects WHERE user_id = "{user_data['user_id']}" """))]
        if len(count_data) != 0:
